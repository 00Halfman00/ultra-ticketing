...
export class Password {
  static async toHash(password: string) {
    try {
      const saltBuf = await randomBytes(16); // vs code infers that this is of type: Buffer, same as below
      const passwordBuf = (await scryptAsync(
        password,
        saltBuf,
        keylen
      )) as Buffer; // explicitly typed as Buffer casue the promisy throws off type inference in VSCode

      return `${passwordBuf.toString(hex)}.${saltBuf.toString(hex)}`;
    } catch (err) {
      console.error(err);
